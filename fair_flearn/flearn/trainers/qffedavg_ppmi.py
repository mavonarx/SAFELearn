...
class Server(BaseFedarated):
    ...
    def train(self):
        ...
        for client_index, c in enumerate(selected_clients):
            ...
            weights_before = np.concatenate((weights_before[0].ravel().reshape(-1,), weights_before[0][0].ravel().reshape(-1,), weights_before[1].ravel().reshape(-1,), weights_before[1][1].ravel().reshape(-1,)))
